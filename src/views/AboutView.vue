    ...
    <div class="container">
      ...
      <div class="row align-items-start mark-row grabbable">
        <div :class="col.cs" v-for="(col, colIndex) in row.cols" :key="col.id" draggable="true" @dragstart="dragStart($event, col)">
          <div>{{col.cs}} S:{{col.start}} E:{{col.end}}</div>		
        </div>
      </div>
    </div>
  </template>
...
export default {
  ...
  computed: {
    rows: function() {
        ...
        rows.forEach(row => {
          ...
          cols.forEach(col => {
            let cs = "";
            if(col.start>1 && col.start!=(lastEnd+1)){
              let width = col.end-col.start+1;
              cs = "offset-"+(lastEnd-1)+" col-"+width.toString();
            }else if(col.start>1){
              let width = col.end-col.start+1;
              cs = "offset-"+(lastEnd-1)+" col-"+width.toString();
            }else{
              let width = col.end-col.start+1;
              cs = "col-"+width.toString();
            }
            lastEnd = col.end;
  ...
  methods: {
    ...
    drop: function(event, rowId, start) {
      event.preventDefault();
      var colId = event.dataTransfer.getData("Text");
...
class RowRepo {
			...
			this.rows = [
        {
          id:4,
          cols: [
            {id: 2, start:2, end:3, content:'test', cs:''},
          ]
        },
        {
          id:0,
          cols: [
            {id: 0, start:1, end:2, content:'test', cs:''},
            {id: 1, start:4, end:6, content:'test2', cs:''}
          ]
        },
        {
          id:1,
          cols: [
            {id: 2, start:1, end:2, content:'test', cs:''},
            {id: 3, start:3, end:5, content:'test2', cs:''},
            {id: 4, start:6, end:12, content:'test3', cs:''}
          ]
        },
        {
          id:3,
          cols: [
            {id: 2, start:4, end:5, content:'test', cs:''},
          ]
        },
      ];
		}
